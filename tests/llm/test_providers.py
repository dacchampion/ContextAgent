...
async def test_gemini_provider(mock_model_class, mock_settings):
    ...
    assert result.summary == VALID_JSON_RESPONSE["summary"]
    assert result.bias_score == VALID_JSON_RESPONSE["bias_score"]
...
async def test_openai_provider(mock_async_openai_class, mock_settings):
    ...
    mock_message = MagicMock(content=VALID_JSON_STRING)
    ...
    assert isinstance(result, NarrativeResponse)
    assert result.summary == VALID_JSON_RESPONSE["summary"]
    assert result.key_levels == VALID_JSON_RESPONSE["key_levels"]
